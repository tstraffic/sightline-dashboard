...
**What's broken or fake:** **The offline banner lies.** The PWA shows "data will sync when you're back online" (offline-queue-banner partial), but [public/worker-sw.js:93-100](public/worker-sw.js:93) only implements a `wq-flush` postMessage tag for reads — there is no write queue. A docket signed offline is lost on submit. Old `/w/timesheets` route still exists ([routes/worker/timesheets.js](routes/worker/timesheets.js)) with a fully-functional form but is redirect-shadowed at [server.js:213](server.js:213) — workers who bookmarked the old URL get redirected, but the form itself still posts if called directly.
**Effort to make it REAL:** S — remove the misleading offline banner OR implement a real IndexedDB queue (M).
**Red flags:** A buyer's IT person opens DevTools, goes offline, signs a test docket — submission fails silently and the UI claims it'll sync.
...
**What's broken or fake:** **No Xero / MYOB / Employment Hero / KeyPay export.** Grep returns zero matches for any of those names. Only output is raw bucket XLSX. **July 2025 3.5% award increase not phased in.** Rates are static snapshots at import time — there's no scheduled rate-rotation logic, no `effective_date` on the seeded award rates. **PAYG tax calc is stubbed** — `payAsYouGo` exported from [lib/payroll.js](lib/payroll.js) but never called by [routes/payroll-runs.js](routes/payroll-runs.js). Pay-run "Approvers" list is hardcoded to fall back to specific names if `saadat`/`sajid` not found — buyer-specific.
...
Three sub-agent findings were checked and corrected during synthesis:
