  ...
  try {
    rows = db.prepare(`
      SELECT jd.id, jd.job_id, jd.doc_type, jd.title, jd.original_name, jd.mime_type,
             ...
             -- Crew-side switch (migration 346). NOT filtered here: admin
             -- surfaces render hidden docs greyed with the toggle.
             COALESCE(jd.visible_to_crew, 1) AS visible_to_crew,
             '/jobs/' || jd.job_id || '/documents/' || jd.id || '/download' AS href
  ...
  try {
    const finals = db.prepare(`
      SELECT tp.id, tp.job_id,
             LOWER(COALESCE(NULLIF(tp.plan_type, ''), 'plan')) AS doc_type,
             ...
             -- Crew-side override (migration 340). NOT filtered here: admin
             -- surfaces must render hidden plans greyed with the toggle.
             COALESCE(tp.visible_to_crew, 1) AS visible_to_crew,
             -- Plan files are served statically from public/ (same link the
             -- job page's Final Plans tab uses); they are NOT job_documents,
             -- so the /jobs/:id/documents/:id/download URL would 404 — and
             -- worse, silently point at an unrelated job's document id.
             CASE WHEN COALESCE(tp.file_path, '') != '' THEN '/' || tp.file_path
                  ELSE '/plans/' || tp.id END AS href
