...
const DEPARTMENTS = {
  planning: {
    ...
    stats(db, today) {
      ...
      return [
        { label: 'Open plans', value: count(db, "SELECT COUNT(*) as c FROM compliance WHERE parent_id IS NULL AND status NOT IN ('approved','expired','rejected')"), tone: 'is-info', href: '/compliance' },
  ...
  reports: {
    ...
    // The reports sidebar section is negated-gate based (Safety/HR Reports
    // only show there when the user LACKS 'reports') — wrong shape for a hub
    // grid, so this hub skips SECTIONS and keeps its own curated links.
    sectionLinks: false,
    extraLinks: [
    ...
    stats(db, today) {
      const { getChartData } = require('../routes/helpers/dashboard-queries');
      const dist = getChartData(db).jobStatusDist;
      const by = {};
      dist.forEach(r => { by[String(r.status || '').toLowerCase()] = r.count; });
      const total = dist.reduce((a, r) => a + r.count, 0);
      return [
        { label: 'Jobs on record', value: total, tone: 'is-info', href: '/reports?tab=jobs' },
        { label: 'Active jobs', value: by.active || 0, tone: 'is-good', href: '/reports?tab=jobs' },
        { label: 'Completed jobs', value: by.completed || 0, tone: 'is-muted', href: '/reports?tab=jobs' },
      ];
    },
  },
};
...
module.exports = { DEPARTMENTS, DEPARTMENT_ORDER, getDepartment, userCanAccessDept, moduleLinks };
