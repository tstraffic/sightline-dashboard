...
//
// accessKeys MUST mirror the matching sidebar section gate in
// views/partials/sidebar.ejs (Planning L97 / Safety L143 / Operations L255 /
// Finance L362 / People L412 / Assets & Reports L461) so the invariant holds:
// "I can see the section in the sidebar" ⇔ "I can open its hub". Edit both
// places together.
//
...
const DEPARTMENTS = {
  planning: {
    ...
    accessKeys: ['planning_plans', 'projects', 'tenders', 'quoting'],
    quickLinks: [
    ...
    stats(db, today) {
      const overduePlans = count(db, "SELECT COUNT(*) as c FROM compliance WHERE due_date < ? AND status NOT IN ('approved','expired','submitted')", today);
      return [
  ...
  safety: {
    ...
    accessKeys: ['safety_today', 'audits', 'incidents', 'checklists', 'swms', 'sop_register', 'risk_assessments', 'safety_updates', 'toolbox_talks', 'safety_comments', 'safety_quizzes', 'safety_reports', 'voc'],
    heroLink: {
  ...
  operations: {
    ...
    stats(db, today) {
      ...
      const allocatedToday = count(db, "SELECT COUNT(DISTINCT crew_member_id) as c FROM crew_allocations WHERE allocation_date = ?", today);
      const unconfirmed = count(db, "SELECT COUNT(*) as c FROM crew_allocations WHERE allocation_date = ? AND status = 'allocated'", today);
      return [
        { label: 'Active jobs', value: count(db, "SELECT COUNT(*) as c FROM jobs WHERE status = 'active'"), tone: 'is-good', href: '/projects' },
  ...
  finance: {
    ...
    accessKeys: ['finance', 'payroll', 'timesheets', 'budgets'],
    quickLinks: [
  ...
  people: {
    ...
    accessKeys: ['induction', 'hr_dashboard'],
    quickLinks: [
...
function userCanAccessDept(user, dept) {
  return dept.accessKeys.some(k => canAccess(user, k));
}
