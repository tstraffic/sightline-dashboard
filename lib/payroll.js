...
function splitShift(shift, paymentType, isPH, rates) {
  ...
  // TFN Night 5+ — when annotateNightRuns has tagged this shift as
  // part of a 5+ Mon–Fri night run, route every hour into the
  // night_5plus bucket. No OT/DT split: the panel defines Night 5+ as
  // a single flat rate per hour, lower than Night<5, so the worker is
  // not penalised by losing the OT premium (the lower base rate
  // already accounts for the discount).
  if (paymentType === 'tfn' && night && shift._night_run_type === '5plus' && toNum(r.night_5plus) > 0) {
    return [{ bucket: 'night_5plus', dow, hours }];
  }
