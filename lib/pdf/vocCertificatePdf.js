...
async function renderVocCertificatePdf(assessment, certId, verifyUrl) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // ─── Sign-off row ──────────────────────────────────────
      // Two columns only — Assessor + Worker Acknowledgement. The
      // Manager/Supervisor middle column was removed because in
      // practice the trainer running the VOC session signs as
      // assessor, and the worker signs as acknowledgement. A separate
      // manager sign-off was rarely captured and just left a blank
      // line in the middle of the cert.
      ...
      // Wider columns now that we only have two — gap of 80pt between
      // them, same outer offset as before so the row stays centred.
      const signGap = 80;
