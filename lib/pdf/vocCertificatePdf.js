...
async function renderVocCertificatePdf(assessment, certId, verifyUrl) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Subtle watermark — T&S logo rotated, centered, faint. Falls
      // back to the text "T&S" if the logo file is missing on disk
      // (defensive — Railway volumes have lost the public/ folder
      // before during a misconfigured deploy).
      doc.save();
      doc.opacity(0.06);
      doc.rotate(-22, { origin: [W / 2, H / 2] });
      if (fs.existsSync(LOGO_PATH)) {
        // Sized so the bounding box mirrors the previous "T&S" text
        // footprint (~480 wide). PDFKit centers via the rect — we offset
        // by half the width so the logo's middle lands at the page
        // centre after the rotation transform.
        const watermarkW = 480;
        const watermarkH = 320;
        doc.image(LOGO_PATH, W / 2 - watermarkW / 2, H / 2 - watermarkH / 2, {
          fit: [watermarkW, watermarkH],
          align: 'center',
          valign: 'center',
        });
      } else {
        doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(180);
