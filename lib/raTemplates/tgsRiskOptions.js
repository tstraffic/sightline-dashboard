...
// 5×5 risk matrix used by the standalone TGS Risk Assessment form under Plans.
// Authoritative copy — both client-side JS (public/js/tgs-risk-assessment.js)
// and the server-side residual-risk recompute use this same mapping.
const LIKELIHOOD = [
  { code: 'A', label: 'Almost Certain' },
  { code: 'B', label: 'Likely' },
  { code: 'C', label: 'Possible' },
  { code: 'D', label: 'Unlikely' },
  { code: 'E', label: 'Rare' },
];

const CONSEQUENCE = [
  { code: 1, label: 'Insignificant' },
  { code: 2, label: 'Minor' },
  { code: 3, label: 'Major' },
  { code: 4, label: 'Severe' },
  { code: 5, label: 'Catastrophic' },
];
...
const RATING_RANK = { Low: 1, Medium: 2, High: 3, Extreme: 4 };

function computeRating(likelihood, consequence) {
  if (!likelihood || !consequence) return null;
  const row = RISK_MATRIX[likelihood];
  if (!row) return null;
  const idx = parseInt(consequence, 10) - 1;
  return row[idx] || null;
}

// Walk a list of risk-management entries (each carries residualL + residualC,
// or likelihood + consequence depending on caller shape) and return the
// highest-band rating across the table — used to drive One-Up Manager gating.
function computeResidualRisk(controls) {
  if (!Array.isArray(controls)) return null;
  let max = null;
  for (const c of controls) {
    const r = computeRating(c.residualL || c.likelihood, c.residualC || c.consequence);
    if (!r) continue;
    if (!max || RATING_RANK[r] > RATING_RANK[max]) max = r;
  }
  return max;
}
