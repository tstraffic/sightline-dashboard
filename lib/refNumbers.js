...
function generateServicePackageRef(jobNumber, stream) {
  const db = getDb();
  const streamKey = String(stream || '').toUpperCase();
  const prefix = `${jobNumber}-${streamKey}-`;
  const row = db.prepare(`
    SELECT MAX(CAST(SUBSTR(package_ref, ?) AS INTEGER)) AS maxSeq
    FROM service_packages
    WHERE package_ref LIKE ? AND SUBSTR(package_ref, ?) GLOB '[0-9]*'
  `).get(prefix.length + 1, prefix + '%', prefix.length + 1);
  let seq = (row && Number.isFinite(row.maxSeq) ? row.maxSeq : 0) + 1;
  const used = db.prepare('SELECT 1 FROM service_packages WHERE package_ref = ?');
  for (let i = 0; i < 1000; i++) {
    const candidate = prefix + String(seq).padStart(2, '0');
    if (!used.get(candidate)) return candidate;
    seq++;
  }
  throw new Error(`Could not allocate a package ref under ${prefix}`);
}
