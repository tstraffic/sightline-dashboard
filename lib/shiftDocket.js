...
/**
 * Generate the next sequential docket number — `TS-DK-00001`, `TS-DK-00002`, …
 *
 * Same pattern as routes/bookings.generateBookingNumber: ignore docket numbers
 * that don't match our prefix (legacy 'DK-NNNN' from booking_dockets, Traffio
 * imports, manual entries) by restricting MAX() to the `TS-DK-` namespace, and
 * self-heal by stepping forward if the candidate is somehow already taken.
 *
 * Pads to 5 digits so we don't run out of space before year ~2030.
 */
function generateDocketNumber(db) {
  const row = db.prepare(`
    SELECT MAX(CAST(SUBSTR(docket_number, 7) AS INTEGER)) AS maxNum
    FROM docket_signatures
    WHERE docket_number LIKE 'TS-DK-%'
      AND SUBSTR(docket_number, 7) GLOB '[0-9]*'
  `).get();
  let next = (row && Number.isFinite(row.maxNum) ? row.maxNum : 0) + 1;
  const check = db.prepare('SELECT 1 AS x FROM docket_signatures WHERE docket_number = ?');
  for (let i = 0; i < 100000; i++) {
    const candidate = 'TS-DK-' + String(next).padStart(5, '0');
    if (!check.get(candidate)) return candidate;
    next++;
  }
  throw new Error('No free docket number after 100000 attempts');
}
...
module.exports = { calcHours, resolveShift, getCurrentDocket, getDocketCrew, completeShift, generateDocketNumber, BOOKING_TERMINAL };
