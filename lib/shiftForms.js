  ...
  try {
    ...
    const crew = db.prepare(`
      SELECT bc.id AS booking_crew_id, bc.crew_member_id, bc.assigned_vehicle_id, bc.role_on_site, bc.status,
             COALESCE(bc.is_team_leader, 0) AS is_team_leader,
             COALESCE(bc.is_first_aid, 0) AS is_first_aid,
             COALESCE(bc.straight_to_site, 0) AS straight_to_site,
    ...
    try {
      ...
      `).all(bookingId).forEach(g => {
        ...
        // Gear not hitched to any ute still travels with the crew — it used
        // to be filtered out entirely, so signs/boards on the booking never
        // appeared in the portal at all.
        if (!g.attached_vehicle_id) { looseGear.push(shaped); return; }
        if (!gearByVehicle.has(g.attached_vehicle_id)) gearByVehicle.set(g.attached_vehicle_id, []);
        gearByVehicle.get(g.attached_vehicle_id).push(shaped);
      });
    ...
    return { groups, onFoot, looseGear };
  } catch (e) { return null; }
}
