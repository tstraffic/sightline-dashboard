...
function mapPresetToEmployeeRates(preset, paymentType, allowances, opts) {
  ...
  if (pt === 'cash') {
    return {
      ...
      // Meal + travel are TFN-only award allowances — never stamped for Cash.
      rate_meal: 0,
      rate_fares_daily: 0,
    };
  }
  ...
  if (pt === 'abn') {
    return {
      ...
      // Meal + travel are TFN-only award allowances — never stamped for ABN.
      rate_meal: 0,
      rate_fares_daily: 0,
    };
  }
  ...
  if (pt === 'tfn') {
    ...
    // from rate_night (Night<5, 1.5×) to the lower 5+ rate (1.15×).
    // For 'permanent' the rate_night is already the 1.3× figure, and for
    // 'continuous_5plus' it's already the 1.15× figure, so there's
    // nothing to promote — leaving rate_night_5plus at 0 keeps the
    // engine's detector idle.
    const np = NIGHT_PATTERNS.includes(nightPattern) ? nightPattern : 'occasional';
    return {
      rate_day: base,
      // BCG casual OT Mon–Fri per the FWC pay guide: first 2 hours pays the
      // same loaded figure as Sat ≤2h, after 2 hours the same as Sat >2h
      // (1.5× / 2× on the UNLOADED ordinary rate + casual loading — NOT
      // base × 1.5, which overpays because base already carries the 25%
      // loading). The preset's sat columns hold exactly those guide values.
      rate_ot: r2(preset.rate_sat_short),
      rate_dt: r2(preset.rate_sat_long),
