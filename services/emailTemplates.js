    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        ...
        <tr><td style="padding:16px 24px;text-align:center;border-radius:0 0 12px 12px;">
          <p style="margin:0;color:#9CA3AF;font-size:12px;">&copy; Sightline Traffic Engineers &middot; Powered by Atomis. This is an automated message.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function buttonHtml(text, url) {
  return `<table cellpadding="0" cellspacing="0" style="margin:24px 0;">
    <tr><td style="background:#A34652;border-radius:8px;padding:12px 28px;">
      <a href="${url}" style="color:#F8F6F2;text-decoration:none;font-weight:600;font-size:15px;display:inline-block;">${text}</a>
    </td></tr>
  </table>`;
}
...
function adminInviteEmail(fullName, inviteUrl, expiresHours) {
  return baseTemplate('You\'ve Been Invited', `
    ...
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">You've been invited to <strong>Atomis</strong>, the operating platform for Sightline Traffic Engineers. Click the button below to set your password and activate your account.</p>
...
function workerInviteEmail(fullName, setupUrl, expiresHours) {
  return baseTemplate('Set Up Your Atomis Crew PIN', `
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 8px;">Hi ${fullName},</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">You've been set up on <strong>Atomis Crew</strong>, Sightline Traffic Engineers's field operations app. Tap the button below to create your sign-in PIN.</p>
...
function sopSignLinkEmail(fullName, signUrl) {
  return baseTemplate('SOP Sign-Off — Action Required', `
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 8px;">Hi ${fullName},</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">As part of working with Sightline Traffic Engineers, we need you to sign off that you've reviewed our Standard Operating Procedures and been adequately educated on them.</p>
    ...
    <p style="color:#6B7280;font-size:13px;margin:0;">If the button doesn't work, copy and paste this link into your browser:<br><a href="${signUrl}" style="color:#8F3D48;word-break:break-all;">${signUrl}</a></p>
  `);
}
...
function contractSignLinkEmail(fullName, signUrl, agreementNumber) {
  return baseTemplate('Your employment agreement — review and sign', `
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 8px;">Hi ${escapeHtml(fullName || 'there')},</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">Your casual employment agreement with Sightline Traffic Engineers (<strong>${escapeHtml(agreementNumber)}</strong>) is ready for you to review and sign.</p>
...
function contractSignedEmail(fullName, agreementNumber) {
  return baseTemplate('Your signed employment agreement', `
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 8px;">Hi ${escapeHtml(fullName || 'there')},</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">Thanks — your casual employment agreement with Sightline Traffic Engineers (<strong>${escapeHtml(agreementNumber)}</strong>) has been signed. Your copy is attached to this email; keep it for your records.</p>
    ...
    <p style="color:#6B7280;font-size:13px;margin:0;">If anything in the agreement looks wrong, contact the Sightline office straight away.</p>
  `);
}
...
function toolboxClientEmail(recipientName, toolbox, message, senderName) {
  ...
  return baseTemplate('Toolbox Talk Record', `
    ...
    <p style="color:#6B7280;font-size:13px;margin:0;">Sent by ${escapeHtml(senderName || 'Sightline Traffic Engineers')} via Atomis. Worker attendance signatures are embedded in the attached PDF.</p>
  `);
}
...
function inductionConfirmationEmail(whenText, inductionUrl) {
  return baseTemplate('Induction Confirmation', `
    ...
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">This is Suhail from Sightline Traffic Engineers.</p>
    ...
    <p style="color:#6B7280;font-size:13px;line-height:1.6;margin:0 0 16px;word-break:break-all;">Or paste this link into your browser: <a href="${inductionUrl}" style="color:#8F3D48;">${inductionUrl}</a></p>
...
function inductionReminderEmail(whenText, inductionUrl, leadLabel) {
  return baseTemplate('Induction Reminder', `
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">Hi,</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">A friendly reminder from Sightline Traffic Engineers — your induction is <strong>${escapeHtml(leadLabel)}</strong>.</p>
    ...
    ${buttonHtml('Open Induction Form', inductionUrl)}
    <p style="color:#6B7280;font-size:13px;line-height:1.6;margin:0 0 16px;word-break:break-all;">Or paste this link into your browser: <a href="${inductionUrl}" style="color:#8F3D48;">${inductionUrl}</a></p>
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0;">See you there — Sightline Traffic Engineers</p>
  `);
}
